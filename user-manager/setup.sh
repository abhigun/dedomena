...
terraform init -backend-config="resource_group_name=${TF_RESOURCE_GROUP}" -backend-config="storage_account_name=${TF_STORAGE_ACCOUNT}" -backend-config="container_name=${TF_STORAGE_CONTAINER}" -backend-config="key=usermanager.tfstate"
...
for name in $users;
do
  echo "$name: $(az keyvault secret show --name $name  --query 'value' --output tsv)"
done
