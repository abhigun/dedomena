...
first_party_users=$(yq eval '.first_party_details.users[].user_principal_name' "$LOCAL_VALUES")
partner_users=$(yq eval '.partner_details.users[].user_principal_name' "$LOCAL_VALUES")
keyvault=$(yq eval '.key_vault_name' "$LOCAL_VALUES" | sed 's/"//g')
echo "First Party Azure Entra Users"
for name in $first_party_users;
do
  echo "$name: $(az ad user show --id $name  --output tsv)"
done
